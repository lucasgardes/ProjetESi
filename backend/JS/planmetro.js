var stopsData = JSON.parse(document.getElementById('stopsData').textContent);
...
function drawMap() {
    ...
    bicyclesData.forEach(bike => {
        if (bike.path) {
            const stopsForBike = bike.path; // Assume that bike.path contains an array of stop names for the bike's path.
            for (let i = 0; i < stopsForBike.length - 1; i++) {
                const stop1 = stopsForBike[i];
                const stop2 = stopsForBike[i + 1];
                ...
                if (station1 && station2) {
                    drawLine(
                        ...
                        station2.x,
                        station2.y,
                        'blue', // Utilisation d'une couleur unique pour les trajets des vélos
                        'bikeRoute',
                        0
                    );
                }
            ...
            stopsForBike.forEach(stop => {
                const station = getStationByName(stop.name);
                if (station) {
                    drawStation(station.x, station.y, station.name, 'bikeRoute', 0);
                }
            });
        }
    });
    ...
    bicyclesData.forEach(bike => {
        ...
        Object.entries(data.lines).forEach(([lineName, line]) => {
            const station = line.stations.find(
                s => s.name.toUpperCase().replace(/[- ]/g, '_') === bike.name
            );
            if (station) {
                stationFound = station;
            }
